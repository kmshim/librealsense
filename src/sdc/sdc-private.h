...
namespace librealsense
{
    namespace sdc
    {
		const uint16_t SDC30_PID        = 0xFF07; // Shinoh Elec

        // SDC depth XU identifiers
        const uint8_t SDC_HWMONITOR                       = 1;
        const uint8_t SDC_DEPTH_EMITTER_ENABLED           = 2;
        const uint8_t SDC_EXPOSURE                        = 3;
        const uint8_t SDC_LASER_POWER                     = 4;
        const uint8_t SDC_HARDWARE_PRESET                 = 6;
        const uint8_t SDC_ERROR_REPORTING                 = 7;
        const uint8_t SDC_EXT_TRIGGER                     = 8;
        const uint8_t SDC_ASIC_AND_PROJECTOR_TEMPERATURES = 9;
        const uint8_t SDC_ENABLE_AUTO_WHITE_BALANCE       = 0xA;
        const uint8_t SDC_ENABLE_AUTO_EXPOSURE            = 0xB;

        // Devices supported by the current version
        static const std::set<std::uint16_t> sdc_sku_pid = {
			sdc::SDC30_PID,
        };
        ...
        static const std::map<std::uint16_t, std::string> rsSDC30_sku_names = {
			{ SDC30_PID,        "Shinoh SDC30"}

        };

        // SDC fisheye XU identifiers
       ...
       // subdevice[h] unit[fw], node[h] guid[fw]
        ...
        enum fw_cmd : uint8_t
        {
            ...
            MMER            = 0x4F,     // MM EEPROM read ( from SDC cache )
        ...
        enum class sdc30_caps : uint16_t
        {
            CAP_UNDEFINED               = 0,
        ...
        static const std::map<sdc30_caps, std::string> d400_capabilities_names = {
            { sdc30_caps::CAP_UNDEFINED,        "Undefined"         },
            { sdc30_caps::CAP_ACTIVE_PROJECTOR, "Active Projector"  },
            { sdc30_caps::CAP_RGB_SENSOR,       "RGB Sensor"        },
            { sdc30_caps::CAP_FISHEYE_SENSOR,   "Fisheye Sensor"    },
            { sdc30_caps::CAP_IMU_SENSOR,       "IMU Sensor"        },
            { sdc30_caps::CAP_GLOBAL_SHUTTER,   "Global Shutter"    },
            { sdc30_caps::CAP_ROLLING_SHUTTER,  "Rolling Shutter"   }
        };

        inline sdc30_caps operator &(const sdc30_caps lhs, const sdc30_caps rhs)
        {
            return static_cast<sdc30_caps>(static_cast<uint32_t>(lhs) & static_cast<uint32_t>(rhs));
        }
        ...
        inline sdc30_caps& operator |=(sdc30_caps& lhs, sdc30_caps rhs)
        {
            return lhs = lhs | rhs;
        }

        inline std::ostream& operator <<(std::ostream& stream, const sdc30_caps& cap)
        {
            for (auto i : { sdc30_caps::CAP_ACTIVE_PROJECTOR,sdc30_caps::CAP_RGB_SENSOR,
							sdc30_caps::CAP_FISHEYE_SENSOR,  sdc30_caps::CAP_IMU_SENSOR,
							sdc30_caps::CAP_GLOBAL_SHUTTER,  sdc30_caps::CAP_ROLLING_SHUTTER })
            {
                if (i==(i&cap))
        ...
        enum sdc_rect_resolutions : unsigned short
        {
            res_320_240, //240x Pixel row data
			res_320_242, //1x MetaData1 line (optional) + 240x Pixel row data + 1x MetaData2 line (optional)
			res_640_480, //1x MetaData1 line (optional) + 240x Pixel row data + 8x MLX75024 Test Rows (optional) + 1x MetaData2 line (optional) --> 처리속도 때문에, 246line까지만 가능함.
			reserved_1,
			reserved_2,
			reserved_3,
			reserved_4,
        ...
        struct coefficients_table
        {
            ...
            uint32_t            brown_model;                //  Distortion model: 0 - SDC distorion model, 1 - Brown model
            uint8_t             reserved1[88];
            float4              rect_params[max_sdc_rect_resolutions];
...
#pragma pack(pop)
        ...
        struct sdc_calibration
        {
            ...
            sdc_calibration() : version(0), left_imager_intrinsic({}), right_imager_intrinsic({}),
                left_imager_extrinsic({}), right_imager_extrinsic({}), depth_extrinsic({})
            {
                for (auto i = 0; i < max_sdc_rect_resolutions; i++)
                    depth_intrinsic[i] = {};
        ...
        static std::map< sdc_rect_resolutions, int2> resolutions_list = {
            { res_320_240,{ 320, 240 } },
			{ res_320_242,{ 320, 242 } },
            { res_640_480,{ 640, 480 } },
        };
        ...
        sdc_rect_resolutions width_height_to_sdc_rect_resolutions(uint32_t width, uint32_t height);
        ...
        const std::map< uint8_t, std::string> sdc_fw_error_report = {
            { success,                      "Success" },
        ...
        std::vector<platform::uvc_device_info> filter_device_by_capability(const std::vector<platform::uvc_device_info>& devices, sdc30_caps caps);
