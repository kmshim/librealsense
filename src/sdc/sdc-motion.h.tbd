...
{
    class sdc_mm_calib_parser
    {
    public:
        virtual rs2_extrinsics get_extrinsic_to(rs2_stream) = 0;    // Extrinsics are referenced to the Depth stream, except for TM1
        virtual sdc::imu_intrinsic get_intrinsic(rs2_stream) = 0;    // With extrinsic from FE<->IMU only
    };

	class sdc_tm1_imu_calib_parser : public sdc_mm_calib_parser
	{
		public:
		sdc_tm1_imu_calib_parser(const std::vector<uint8_t>& raw_data)
		{
			calib_table = *(sdc::check_calib<sdc::tm1_eeprom>(raw_data));
		};
		sdc_tm1_imu_calib_parser(const sdc_tm1_imu_calib_parser&);
		~sdc_tm1_imu_calib_parser() {};
		...
		sdc::imu_intrinsic get_intrinsic(rs2_stream stream)
		{
			sdc::imu_intrinsics in_intr;
			switch (stream)
			{
		...
		private:
		sdc::tm1_eeprom  calib_table;
	};

    class sdc_dm_v2_imu_calib_parser : public sdc_mm_calib_parser
    {
    public:
        sdc_dm_v2_imu_calib_parser(const std::vector<uint8_t>& raw_data, bool valid = true)
        {
            ...
            if (valid)
                calib_table = *(sdc::check_calib<sdc::dm_v2_eeprom>(raw_data));
        };
        sdc_dm_v2_imu_calib_parser(const sdc_dm_v2_imu_calib_parser&);
        ~sdc_dm_v2_imu_calib_parser() {};
        ...
        sdc::imu_intrinsic get_intrinsic(rs2_stream stream)
        {
            ...
            sdc::dm_v2_imu_intrinsic in_intr;
            switch (stream)
            {
                case RS2_STREAM_ACCEL:
    ...
    private:
        sdc::dm_v2_eeprom  calib_table;
    };

    class sdc_mm_calib_handler
	{
	public:
		sdc_mm_calib_handler(std::shared_ptr<hw_monitor> hw_monitor);
		sdc_mm_calib_handler(const sdc_mm_calib_handler&);
		~sdc_mm_calib_handler() {};

		sdc::imu_intrinsic get_intrinsic(rs2_stream);
    ...
    private:
        ...
        lazy< std::shared_ptr<sdc_mm_calib_parser>> _calib_parser;
    ...
    class sdc_motion : public virtual sdc_device
    {
    public:
        ...
        sdc_motion(std::shared_ptr<context> ctx,
    ...
    private:
        ...
        friend class sdc_fisheye_sensor;
        friend class sdc_hid_sensor;
        ...
        std::shared_ptr<sdc_mm_calib_handler>       _mm_calib;
        lazy<sdc::imu_intrinsic>                _accel_intrinsic;
        lazy<sdc::imu_intrinsic>                _gyro_intrinsic;
        lazy<std::vector<uint8_t>>              _fisheye_calibration_table_raw;
