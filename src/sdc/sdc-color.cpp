...
{
    sdc_color::sdc_color(std::shared_ptr<context> ctx,
          ...
          _color_stream(new stream(RS2_STREAM_COLOR))
    {
        ...
        if (color_devs_info.size() != 1)
            throw invalid_value_exception(to_string() << "SDC30 with RGB models are expected to include a single color device! - "
                << color_devs_info.size() << " found");

        auto color_ep = create_color_device(ctx, color_devs_info);
    }

    std::shared_ptr<uvc_sensor> sdc_color::create_color_device(std::shared_ptr<context> ctx,
        const std::vector<platform::uvc_device_info>& color_devices_info)
    {
        ...
        auto color_ep = std::make_shared<sdc_color_sensor>(this, backend.create_uvc_device(color_devices_info.front()),
            std::unique_ptr<frame_timestamp_reader>(new sdc_timestamp_reader_from_metadata(move(sdc_timestamp_reader_backup))));
        ...
        color_ep->register_metadata(RS2_FRAME_METADATA_ACTUAL_FPS,  std::make_shared<sdc_md_attribute_actual_fps> (false, [](const rs2_metadata_type& param)
        ...
        // Starting with firmware 19.11.7.0, auto-exposure ROI is available for color sensor
		//
        if (_fw_version >= firmware_version("20.11.7.0"))
        {
            roi_sensor_interface* roi_sensor;
            if ((roi_sensor = dynamic_cast<roi_sensor_interface*>(color_ep.get())))
                roi_sensor->set_roi_method(std::make_shared<sdc_auto_exposure_roi_method>(*_hw_monitor, sdc::fw_cmd::SETRGBAEROI));
        }

        return color_ep;
    }

    rs2_intrinsics sdc_color_sensor::get_intrinsics(const stream_profile& profile) const
    {
        return get_intrinsic_by_resolution(
            *_owner->_color_calib_table_raw,
            sdc::calibration_table_id::rgb_calibration_id,
            profile.width, profile.height);
    }

    stream_profiles sdc_color_sensor::init_stream_profiles()
    {
        ...
        for (auto p : results)
        {
            ...
            std::weak_ptr<sdc_color_sensor> wp =
                std::dynamic_pointer_cast<sdc_color_sensor>(this->shared_from_this());
    ...
    processing_blocks sdc_color_sensor::get_recommended_processing_blocks() const
    {
