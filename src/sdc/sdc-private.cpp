...
namespace librealsense
{
    namespace sdc
    {
        sdc_rect_resolutions width_height_to_sdc_rect_resolutions(uint32_t width, uint32_t height)
        {
            ...
            return max_sdc_rect_resolutions;
        }
        ...
        {
            auto table = check_calib<sdc::coefficients_table>(raw_data);

			LOG_DEBUG(endl
				<< "baseline = " << table->baseline << " mm" << endl
				<< "Rect params:  \t fX\t\t fY\t\t ppX\t\t ppY \n"
				<< intrinsics_string(res_320_240)
				<< intrinsics_string(res_320_242)
                << intrinsics_string(res_640_480));

            auto resolution = width_height_to_sdc_rect_resolutions(width, height);
            ...
            {
                resolution = width_height_to_sdc_rect_resolutions(width, 480);
            }

            if (resolution < max_sdc_rect_resolutions)
            {
                ...
                // SDCU - assume ideal intrinsic params
            ...
            else
            {
                ...
                sdc_rect_resolutions resolution = res_640_480;
                rs2_intrinsics intrinsics;
        ...
        {
             auto table = check_calib<sdc::fisheye_calibration_table>(raw_data);
        ...
        {
             auto table = check_calib<sdc::rgb_calibration_table>(raw_data);
			...
			sdc30_caps caps)
        {
            ...
            switch (caps)
            {
                case sdc30_caps::CAP_FISHEYE_SENSOR:
