...
namespace librealsense
{
    ...
    class sdc_hid_sensor : public hid_sensor
    {
    public:
        explicit sdc_hid_sensor(sdc_motion* owner, std::shared_ptr<platform::hid_device> hid_device,
    ...
    private:
        const sdc_motion* _owner;
    };

    class sdc_fisheye_sensor : public uvc_sensor, public video_sensor_interface, public roi_sensor_base
    {
    public:
        explicit sdc_fisheye_sensor(sdc_motion* owner, std::shared_ptr<platform::uvc_device> uvc_device,
        ...
        rs2_intrinsics get_intrinsics(const stream_profile& profile) const override
        {
            return get_intrinsic_by_resolution(
                *_owner->_fisheye_calibration_table_raw,
                sdc::calibration_table_id::fisheye_calibration_id,
        ...
        stream_profiles init_stream_profiles() override
        {
            ...
            for (auto p : results)
            {
                ...
                std::weak_ptr<sdc_fisheye_sensor> wp =
                    std::dynamic_pointer_cast<sdc_fisheye_sensor>(this->shared_from_this());
    ...
    rs2_motion_device_intrinsic sdc_motion::get_motion_intrinsics(rs2_stream stream) const
    {
    ...
    std::shared_ptr<hid_sensor> sdc_motion::create_hid_device(std::shared_ptr<context> ctx,
                                                                ...
                                                                const firmware_version& camera_fw_version)
    {
        ...
        auto hid_ep = std::make_shared<sdc_hid_sensor>(this, ctx->get_backend().create_hid_device(all_hid_infos.front()),
                                                        std::unique_ptr<frame_timestamp_reader>(new iio_hid_timestamp_reader()),
                                                        std::unique_ptr<frame_timestamp_reader>(new sdc_custom_hid_timestamp_reader()),
        ...
        if ((camera_fw_version >= firmware_version(custom_sensor_fw_ver))) // && (!val_in_range(pid, { sdc::RS400_IMU_PID, sdc::RS435I_PID, sdc::RS430I_PID })))
        {
            hid_ep->register_option(RS2_OPTION_MOTION_MODULE_TEMPERATURE,
                                    std::make_shared<sdc_motion_module_temperature_option>(*hid_ep));
    ...
    std::shared_ptr<auto_exposure_mechanism> sdc_motion::register_auto_exposure_options(uvc_sensor* uvc_ep, const platform::extension_unit* fisheye_xu)
    {
        ...
        auto exposure_option =  std::make_shared<uvc_xu_option<uint16_t>>(*uvc_ep,
                *fisheye_xu,
                librealsense::sdc::FISHEYE_EXPOSURE, "Exposure time of Fisheye camera");
        ...
        auto auto_exposure_option = std::make_shared<sdc_enable_auto_exposure_option>(uvc_ep,
                                                                                  auto_exposure,
                                                                                  ae_state,
        ...
        uvc_ep->register_option(RS2_OPTION_AUTO_EXPOSURE_MODE,
                                std::make_shared<sdc_auto_exposure_mode_option>(auto_exposure,
                                                                            ae_state,
        ...
        uvc_ep->register_option(RS2_OPTION_AUTO_EXPOSURE_CONVERGE_STEP,
                                std::make_shared<sdc_auto_exposure_step_option>(auto_exposure,
                                                                            ae_state,
                                                                            option_range{ 0.1f, 1.0f, 0.1f, ae_step_default_value }));
        uvc_ep->register_option(RS2_OPTION_POWER_LINE_FREQUENCY,
                                std::make_shared<sdc_auto_exposure_antiflicker_rate_option>(auto_exposure,
                                                                                        ae_state,
    ...
    sdc_motion::sdc_motion(std::shared_ptr<context> ctx,
          ...
          _gyro_stream(new stream(RS2_STREAM_GYRO))
    {
        ...
        _mm_calib = std::make_shared<sdc_mm_calib_handler>(_hw_monitor);
        ...
        if (hid_ep)
        {
            ...
            try
            {
                hid_ep->register_option(RS2_OPTION_ENABLE_MOTION_CORRECTION,
                    std::make_shared<sdc_enable_motion_correction>(hid_ep.get(),
                        *_accel_intrinsic,
                        *_gyro_intrinsic,
                        _depth_to_imu,
    ...
    void sdc_motion::initialize_fisheye_sensor(std::shared_ptr<context> ctx, const platform::backend_device_group& group)
    {
        using namespace sdc;

        auto fisheye_infos = filter_by_mi(group.uvc_devices, 3);
        fisheye_infos = filter_device_by_capability(fisheye_infos, sdc30_caps::CAP_FISHEYE_SENSOR);

        bool fe_dev_present = (fisheye_infos.size() == 1);
        bool fe_capability = (sdc30_caps::CAP_UNDEFINED == _device_capabilities) ?
            true :  !!(static_cast<uint32_t>(_device_capabilities&sdc30_caps::CAP_FISHEYE_SENSOR));
        ...
        std::unique_ptr<frame_timestamp_reader> sdc_timestamp_reader_backup(new sdc_timestamp_reader(environment::get_instance().get_time_service()));
        auto&& backend = ctx->get_backend();
        auto fisheye_ep = std::make_shared<sdc_fisheye_sensor>(this, backend.create_uvc_device(fisheye_infos.front()),
        ...
        if (_fw_version >= firmware_version("19.11.7.0")) // Create Auto Exposure controls from FW version 5.6.3.0
        {
        ...
        else
        {
            ...
            fisheye_ep->register_option(RS2_OPTION_EXPOSURE,
                                        std::make_shared<uvc_xu_option<uint16_t>>(*fisheye_ep.get(),
                                                                                  fisheye_xu,
                                                                                  librealsense::sdc::FISHEYE_EXPOSURE,
	...
	sdc_mm_calib_handler::sdc_mm_calib_handler(std::shared_ptr<hw_monitor> hw_monitor) :  _hw_monitor(hw_monitor)
    {
        _imu_eeprom_raw = [this]() { return get_imu_eeprom_raw(); };

        _calib_parser = [this]() {

            std::vector<uint8_t> raw(sdc::tm1_eeprom_size);
            uint16_t calib_id = sdc::dm_v2_eeprom_id; //assume DM V2 IMU as default platform
            bool valid = false;
            ...
            std::shared_ptr<sdc_mm_calib_parser> prs = nullptr;
            switch (calib_id)
            {
                case sdc::dm_v2_eeprom_id: // DM V2 id
                    prs = std::make_shared<sdc_dm_v2_imu_calib_parser>(raw,valid); break;
                case sdc::tm1_eeprom_id:// TM1 id
                    prs = std::make_shared<sdc_tm1_imu_calib_parser>(raw); break;
                default:
    ...
    std::vector<uint8_t> sdc_mm_calib_handler::get_imu_eeprom_raw() const
    {
        const int offset = 0;
        const int size = sdc::eeprom_imu_table_size;
        command cmd(sdc::MMER, offset, size);
        return _hw_monitor->send(cmd);
    }

    sdc::imu_intrinsic sdc_mm_calib_handler::get_intrinsic(rs2_stream stream)
    {
        return (*_calib_parser)->get_intrinsic(stream);
    }

    rs2_extrinsics sdc_mm_calib_handler::get_extrinsic(rs2_stream stream)
    {
        return (*_calib_parser)->get_extrinsic_to(stream);
    }

    const std::vector<uint8_t> sdc_mm_calib_handler::get_fisheye_calib_raw()
    {
        auto fe_calib_table = (*(sdc::check_calib<sdc::tm1_eeprom>(*_imu_eeprom_raw))).calibration_table.calib_model.fe_calibration;
        uint8_t* fe_calib_ptr = reinterpret_cast<uint8_t*>(&fe_calib_table);
        return std::vector<uint8_t>(fe_calib_ptr, fe_calib_ptr+ sdc::fisheye_calibration_table_size);
    }
}
