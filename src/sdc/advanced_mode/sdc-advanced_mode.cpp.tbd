...
#include "sdc/sdc-active.h"
#include "sdc/sdc-rolling-shutter.h"
#include "sdc-json_loader.hpp"
#include "sdc/sdc-color.h"

namespace librealsense
{
    sdc_advanced_mode_base::sdc_advanced_mode_base(std::shared_ptr<hw_monitor> hwm,
    ...
    {
        _enabled = [this]() {
            auto results = send_receive(encode_command(sdc::fw_cmd::UAMG));
            assert_no_error(sdc::fw_cmd::UAMG, results);
            return results[4] > 0;
        };
        _preset_opt = std::make_shared<sdc_advanced_mode_preset_option>(*this,
            _depth_sensor,
            option_range{ 0,
            RS2_SDC_VISUAL_PRESET_COUNT - 1,
            1,
            RS2_SDC_VISUAL_PRESET_CUSTOM });
        ...
        _color_sensor = [this]() {
            ...
            {
                if (auto s = dynamic_cast<const sdc_color_sensor*>(&(dev.get_sensor(i))))
                {
                    return const_cast<sdc_color_sensor*>(s);
                }
            }
            return (sdc_color_sensor*)nullptr;
        };
    }

    bool sdc_advanced_mode_base::is_enabled() const
    {
        return *_enabled;
    }

    void sdc_advanced_mode_base::toggle_advanced_mode(bool enable)
    {
        send_receive(encode_command(sdc::fw_cmd::EN_ADV, enable));
        send_receive(encode_command(sdc::fw_cmd::HWRST));
    }

    void sdc_advanced_mode_base::apply_preset(const std::vector<platform::stream_profile>& configuration,
                                              rs2_sdc_visual_preset preset, uint16_t device_pid,
                                              const firmware_version& fw_version)
    {
        ...
        switch (preset)
        {
        case RS2_SDC_VISUAL_PRESET_DEFAULT:
            switch (device_pid)
            {
			case sdc::SDC30_PID:
				sdc_default(p); 
            ...
            break;
        case RS2_SDC_VISUAL_PRESET_HAND:
            sdc_hand_gesture(p);
            break;
        case RS2_SDC_VISUAL_PRESET_HIGH_ACCURACY:
            switch (res)
            {
            case low_resolution:
                sdc_low_res_high_accuracy(p);
                break;
            case medium_resolution:
				sdc_mid_res_high_accuracy(p);
                break;
            case high_resolution:
				sdc_high_res_high_accuracy(p);
                break;
            }
            break;
        case RS2_SDC_VISUAL_PRESET_HIGH_DENSITY:
            switch (res)
            {
            case low_resolution:
				sdc_low_res_high_density(p);
                break;
            case medium_resolution:
				sdc_mid_res_high_density(p);
                break;
            case high_resolution:
				sdc_high_res_high_density(p);
                break;
            }
            break;
        case RS2_SDC_VISUAL_PRESET_MEDIUM_DENSITY:
            switch (res)
            {
            case low_resolution:
				sdc_low_res_mid_density(p);
                break;
            case medium_resolution:
				sdc_mid_res_mid_density(p);
                break;
            case high_resolution:
				sdc_high_res_mid_density(p);
                break;
            }
            break;
        case RS2_SDC_VISUAL_PRESET_REMOVE_IR_PATTERN:
        {
    ...
    void sdc_advanced_mode_base::get_depth_control_group(STDepthControlGroup* ptr, int mode) const
    {
        *ptr = get<STDepthControlGroup>(advanced_mode_traits<STDepthControlGroup>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_rsm(STRsm* ptr, int mode) const
    {
        *ptr = get<STRsm>(advanced_mode_traits<STRsm>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_rau_support_vector_control(STRauSupportVectorControl* ptr, int mode) const
    {
        *ptr = get<STRauSupportVectorControl>(advanced_mode_traits<STRauSupportVectorControl>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_color_control(STColorControl* ptr, int mode) const
    {
        *ptr = get<STColorControl>(advanced_mode_traits<STColorControl>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_rau_color_thresholds_control(STRauColorThresholdsControl* ptr, int mode) const
    {
        *ptr = get<STRauColorThresholdsControl>(advanced_mode_traits<STRauColorThresholdsControl>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_slo_color_thresholds_control(STSloColorThresholdsControl* ptr, int mode) const
    {
        *ptr = get<STSloColorThresholdsControl>(advanced_mode_traits<STSloColorThresholdsControl>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_slo_penalty_control(STSloPenaltyControl* ptr, int mode) const
    {
        *ptr = get<STSloPenaltyControl>(advanced_mode_traits<STSloPenaltyControl>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_hdad(STHdad* ptr, int mode) const
    {
        *ptr = get<STHdad>(advanced_mode_traits<STHdad>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_color_correction(STColorCorrection* ptr, int mode) const
    {
        *ptr = get<STColorCorrection>(advanced_mode_traits<STColorCorrection>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_depth_table_control(STDepthTableControl* ptr, int mode) const
    {
        *ptr = get<STDepthTableControl>(advanced_mode_traits<STDepthTableControl>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_ae_control(STAEControl* ptr, int mode) const
    {
        *ptr = get<STAEControl>(advanced_mode_traits<STAEControl>::group, nullptr, mode);
    }

    void sdc_advanced_mode_base::get_census_radius(STCensusRadius* ptr, int mode) const
    {
        *ptr = get<STCensusRadius>(advanced_mode_traits<STCensusRadius>::group, nullptr, mode);
    }

    bool sdc_advanced_mode_base::supports_option(const uvc_sensor& sensor, rs2_option opt) const
    {
        return sensor.supports_option(opt);
    }

    void sdc_advanced_mode_base::get_laser_power(sdc_laser_power_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_laser_state(sdc_laser_state_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_exposure(uvc_sensor& sensor, sdc_exposure_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_auto_exposure(uvc_sensor& sensor, sdc_auto_exposure_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_depth_exposure(sdc_exposure_control* ptr) const
    {
        get_exposure(_depth_sensor, ptr);
    }

    void sdc_advanced_mode_base::get_depth_auto_exposure(sdc_auto_exposure_control* ptr) const
    {
        get_auto_exposure(_depth_sensor, ptr);
    }

    void sdc_advanced_mode_base::get_depth_gain(sdc_gain_control* ptr) const
    {
	...
	void sdc_advanced_mode_base::get_depth_gamma(sdc_gamma_control* ptr) const
	{
		if (supports_option(_depth_sensor, RS2_OPTION_GAMMA))
		{
			ptr->gamma = _depth_sensor.get_option(RS2_OPTION_GAMMA).query();
			ptr->was_set = true;
		}
	}

    void sdc_advanced_mode_base::get_depth_auto_white_balance(sdc_auto_white_balance_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_exposure(sdc_exposure_control* ptr) const
    {
        if (*_color_sensor)
        {
    ...
    void sdc_advanced_mode_base::get_color_auto_exposure(sdc_auto_exposure_control* ptr) const
    {
        if (*_color_sensor)
        {
    ...
    void sdc_advanced_mode_base::get_color_backlight_compensation(sdc_backlight_compensation_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_brightness(sdc_brightness_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_contrast(sdc_contrast_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_gain(sdc_gain_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_gamma(sdc_gamma_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_hue(sdc_hue_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_saturation(sdc_saturation_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_sharpness(sdc_sharpness_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_white_balance(sdc_white_balance_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_auto_white_balance(sdc_auto_white_balance_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::get_color_power_line_frequency(sdc_power_line_frequency_control* ptr) const
    {
    ...
    void sdc_advanced_mode_base::set_depth_control_group(const STDepthControlGroup& val)
    {
        set(val, advanced_mode_traits<STDepthControlGroup>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_rsm(const STRsm& val)
    {
        set(val, advanced_mode_traits<STRsm>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_rau_support_vector_control(const STRauSupportVectorControl& val)
    {
        set(val, advanced_mode_traits<STRauSupportVectorControl>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_color_control(const STColorControl& val)
    {
        set(val, advanced_mode_traits<STColorControl>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_rau_color_thresholds_control(const STRauColorThresholdsControl& val)
    {
        set(val, advanced_mode_traits<STRauColorThresholdsControl>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_slo_color_thresholds_control(const STSloColorThresholdsControl& val)
    {
        set(val, advanced_mode_traits<STSloColorThresholdsControl>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_slo_penalty_control(const STSloPenaltyControl& val)
    {
        set(val, advanced_mode_traits<STSloPenaltyControl>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_hdad(const STHdad& val)
    {
        set(val, advanced_mode_traits<STHdad>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_color_correction(const STColorCorrection& val)
    {
        set(val, advanced_mode_traits<STColorCorrection>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_depth_table_control(const STDepthTableControl& val)
    {
        set(val, advanced_mode_traits<STDepthTableControl>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_ae_control(const STAEControl& val)
    {
        set(val, advanced_mode_traits<STAEControl>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_census_radius(const STCensusRadius& val)
    {
        set(val, advanced_mode_traits<STCensusRadius>::group);
        _preset_opt->set(RS2_SDC_VISUAL_PRESET_CUSTOM);
    }

    void sdc_advanced_mode_base::set_laser_power(const sdc_laser_power_control& val)
    {
        if (val.was_set)
            _depth_sensor.get_option(RS2_OPTION_LASER_POWER).set(val.laser_power);
    }

    void sdc_advanced_mode_base::set_laser_state(const sdc_laser_state_control& val)
    {
        if (val.was_set)
            _depth_sensor.get_option(RS2_OPTION_EMITTER_ENABLED).set((float)val.laser_state);
    }

    void sdc_advanced_mode_base::set_exposure(uvc_sensor& sensor, const sdc_exposure_control& val)
    {
        sensor.get_option(RS2_OPTION_EXPOSURE).set(val.exposure);
    }

    void sdc_advanced_mode_base::set_auto_exposure(uvc_sensor& sensor, const sdc_auto_exposure_control& val)
    {
        sensor.get_option(RS2_OPTION_ENABLE_AUTO_EXPOSURE).set(float(val.auto_exposure));
    }

    void sdc_advanced_mode_base::set_depth_exposure(const sdc_exposure_control& val)
    {
        if (val.was_set)
            set_exposure(_depth_sensor, val);
    }

    void sdc_advanced_mode_base::set_depth_auto_exposure(const sdc_auto_exposure_control& val)
    {
        if (val.was_set)
            set_auto_exposure(_depth_sensor, val);
    }

    void sdc_advanced_mode_base::set_depth_gain(const sdc_gain_control& val)
    {
        if (val.was_set)
            _depth_sensor.get_option(RS2_OPTION_GAIN).set(val.gain);
    }

	void sdc_advanced_mode_base::set_depth_gamma(const sdc_gamma_control& val)
	{
		if (val.was_set)
			_depth_sensor.get_option(RS2_OPTION_GAMMA).set(val.gamma);
	}

    void sdc_advanced_mode_base::set_depth_auto_white_balance(const sdc_auto_white_balance_control& val)
    {
        if (val.was_set)
            _depth_sensor.get_option(RS2_OPTION_ENABLE_AUTO_WHITE_BALANCE).set(float(val.auto_white_balance));
    }

    void sdc_advanced_mode_base::set_color_exposure(const sdc_exposure_control& val)
    {
    ...
    void sdc_advanced_mode_base::set_color_auto_exposure(const sdc_auto_exposure_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_backlight_compensation(const sdc_backlight_compensation_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_brightness(const sdc_brightness_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_contrast(const sdc_contrast_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_gain(const sdc_gain_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_gamma(const sdc_gamma_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_hue(const sdc_hue_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_saturation(const sdc_saturation_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_sharpness(const sdc_sharpness_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_white_balance(const sdc_white_balance_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_auto_white_balance(const sdc_auto_white_balance_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    void sdc_advanced_mode_base::set_color_power_line_frequency(const sdc_power_line_frequency_control& val)
    {
        if (val.was_set && !*_color_sensor)
    ...
    std::vector<uint8_t> sdc_advanced_mode_base::serialize_json() const
    {
        if (!is_enabled())
            throw wrong_api_call_sequence_exception(to_string() << "serialize_json() failed! Device is not in Advanced-Mode.");

        auto p = get_all();
        return sdc_generate_json(p);
    }

    void sdc_advanced_mode_base::load_json(const std::string& json_content)
    {
        if (!is_enabled())
    ...
    sdc_preset sdc_advanced_mode_base::get_all() const
    {
		sdc_preset p;
    ...
    void sdc_advanced_mode_base::set_all(const sdc_preset& p)
    {
    ...
    std::vector<uint8_t> sdc_advanced_mode_base::send_receive(const std::vector<uint8_t>& input) const
    {
    ...
    uint32_t sdc_advanced_mode_base::pack(uint8_t c0, uint8_t c1, uint8_t c2, uint8_t c3)
    {
        return (c0 << 24) | (c1 << 16) | (c2 << 8) | c3;
    }

    std::vector<uint8_t> sdc_advanced_mode_base::assert_no_error(sdc::fw_cmd opcode, const std::vector<uint8_t>& results)
    {
    ...
    std::vector<uint8_t> sdc_advanced_mode_base::encode_command(sdc::fw_cmd opcode,
        uint32_t p1,
        uint32_t p2,
        uint32_t p3,
        uint32_t p4,
    ...
    sdc_advanced_mode_preset_option::sdc_advanced_mode_preset_option(sdc_advanced_mode_base& advanced,
        uvc_sensor& ep, const option_range& opt_range)
        : option_base(opt_range),
        _ep(ep),
        _advanced(advanced),
        _last_preset(RS2_SDC_VISUAL_PRESET_CUSTOM)
    {
        _ep.register_on_open([this](std::vector<platform::stream_profile> configurations) {
            std::lock_guard<std::mutex> lock(_mtx);
            if (_last_preset != RS2_SDC_VISUAL_PRESET_CUSTOM)
                _advanced.apply_preset(configurations, _last_preset, get_device_pid(_ep), get_firmware_version(_ep));
        });
    }

    rs2_sdc_visual_preset sdc_advanced_mode_preset_option::to_preset(float x)
    {
        return (static_cast<rs2_sdc_visual_preset>((int)x));
    }

    void sdc_advanced_mode_preset_option::set(float value)
    {
        std::lock_guard<std::mutex> lock(_mtx);
        if (!is_valid(value))
        ...
        if (preset == RS2_SDC_VISUAL_PRESET_CUSTOM || !_ep.is_streaming())
        {
            _last_preset = preset;
            return;
        }
    ...
    float sdc_advanced_mode_preset_option::query() const
    {
        return static_cast<float>(_last_preset);
    }

    bool sdc_advanced_mode_preset_option::is_enabled() const
    {
        return true;
    }

    const char* sdc_advanced_mode_preset_option::get_description() const
    {
        return "Advanced-Mode Preset";
    }

    const char* sdc_advanced_mode_preset_option::get_value_description(float val) const
    {
        try {
            return rs2_sdc_visual_preset_to_string(to_preset(val));
        }
    ...
    uint16_t sdc_advanced_mode_preset_option::get_device_pid(const uvc_sensor& sensor) const
    {
    ...
    firmware_version sdc_advanced_mode_preset_option::get_firmware_version(const uvc_sensor& sensor) const
    {
